...
Each `SCMSource` assumes that it is owned by an object.
...
+
The listener will need to update the required `SCMSourceOwner` instances on the basis of the event and should minimize full indexing, e.g. by using the `SCMSourceOwner.fetch(..., _event_, _listener_)` variants.
